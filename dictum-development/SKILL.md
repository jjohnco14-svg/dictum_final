...
**If the project isn't pure Dictum — Python does orchestration/I/O/AI
glue, Dictum owns the logic that must be verified — read
`references/GUIDE_POLYGLOT_python_orchestration.md` before writing
anything.** This is a real, working split (not a Dictum backend, none
is planned): a `.dict` kernel compiled to a real `.so`, called from
Python via a binding generated by `dictum emit-binding` — never
hand-written ctypes, which has a real, reproduced silent-wrong-answer
failure mode the guide walks through. Trigger this whenever the
person's request mixes "verified logic" with "CLI/API/glue work," even
if they don't say "Python" explicitly.
